...
import CheckupsExplorer from "@/components/checkups/CheckupsExplorer";
import IndividualCheckups from "@/components/checkups/IndividualCheckups";
import { INDIVIDUAL_CHECKUPS } from "@/lib/individual-checkups";
import { getCheckupPackages, getContactPage } from "@/lib/payload-data";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Checkups" });
  ...
  return {
    ...
    alternates: buildLocalizedAlternates(locale as Locale, "/checkups"),
  };
}
...
export default async function CheckupsPage({
  ...
  const { locale } = await params;
  // `?p=woman|man|child` — deep link from the home teaser cards; preselects
  // the persona so the visitor lands straight on that category's packages.
  const { p } = await searchParams;
  const initialPersona = p === "woman" || p === "man" || p === "child" ? p : null;
  const nav = await getTranslations("Navigation");
  const t = await getTranslations("Checkups");
  const tBook = await getTranslations("Hero");
  const loc = locale as "ge" | "en" | "ru";

  const checkupPackages = await getCheckupPackages(loc);
  const contact = await getContactPage(loc);
  const phone = (contact as { phone?: { value?: string } | null } | null)?.phone?.value ?? "";
    ...
    <>
      <StructuredData
        data={generateBreadcrumbSchema([
          ...
          { name: t("title"), url: `/${locale}/checkups` },
        ])}
      />
      ...
      {checkupPackages.length > 0 ? (
        /* Populated — the explorer renders its own immersive gateway hero
           (it owns the page H1), so no standalone hero section here. */
        <CheckupsExplorer
        ...
        <>
          {/* Hero — solid blackberry with drifting wash (empty-state path) */}
          <section className="relative bg-blackberry py-16 sm:py-20 lg:py-28 overflow-hidden">
            ...
            <div className="relative max-w-4xl mx-auto px-4 sm:px-6 md:px-10">
              ...
              <h1 className="text-[clamp(2.2rem,5vw,4rem)] font-bold text-white tracking-[-0.02em] leading-[1.05] break-words">
                {t("title")}
              </h1>
            </div>
          </section>

          {/* Empty state — typographic, with CTA back to booking */}
          ...
          <div className="max-w-2xl mx-auto px-4 sm:px-6 md:px-10 text-center">
            ...
            <p className="text-[clamp(1.2rem,2vw,1.5rem)] text-blackberry leading-[1.55] font-medium mb-10 break-words">
              {t("subtitle")}
            </p>
            <Link
              href="/booking"
      ...
      {/* Targeted single-condition check-ups — a static catalogue (no audience/
          tier), shown to everyone below the audience packages. */}
      <IndividualCheckups checkups={INDIVIDUAL_CHECKUPS} phone={phone} locale={loc} />
    </>
  );
}
